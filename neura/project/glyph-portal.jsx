 ...
 *
 * Adaptado al proyecto: JSX sin tipos, textos de interfaz en español, `inert` como
 * string (React 18 no lo reconoce como booleano). Opción `glyphs`: la palabra se dibuja
 * con formas vectoriales propias (p. ej. las letras de un logo) en lugar de una tipografía.
 */
...
/** Igual que interior(), para una letra dada como forma vectorial (glyphs). */
function interiorPath(context, d, box) {
  const canvas = context.canvas;
  const pad = 8;
  canvas.width = Math.max(1, Math.ceil(box.width * 3) + pad * 2);
  canvas.height = Math.max(1, Math.ceil(box.height * 3) + pad * 2);
  context.setTransform(3, 0, 0, 3, pad - box.x * 3, pad - box.y * 3);
  context.fill(new Path2D(d), 'evenodd');
  ...
  if (size < 3) return null;
  return { x: (bx + 1 - size / 2 - pad) / 3 + box.x, y: (by + 1 - size / 2 - pad) / 3 + box.y, radius: (size / 2 - 1) / 3 };
}
...
export default function GlyphPortal({
  word = 'NEURA', focusChar, interactive = true, background, front, children, scrollLength = 2.4,
  fontFamily = DEFAULT_FONT, fontWeight = 900, annotations = false,
  enterLabel = 'Entrar', className, style, onProgress,
  // Opcional: borde superior de la palabra en px. Por defecto la palabra se centra al 46%.
  wordTop,
  // Opcional: alto de la palabra en px (el que la llama ya calculó el espacio). Con esto la
  // palabra puede ocupar el 90% del ancho y no se limita al centrado original.
  wordMaxHeight,
  // Opcional: el contenido aparece dentro de la vista fija, sobre el fondo, al terminar el
  // zoom. Sin movimiento (reducido o estático) vuelve a ir debajo, como en el original.
  overlay = false,
  // Opcional: [{ char, d, box: { x, y, width, height } }] en unidades de la palabra (línea de
  // base en y = 0). Reemplaza a la tipografía; una entrada por letra de `word`.
  glyphs,
}) {
  ...
  const text = word.trim().normalize('NFC') || 'NEURA';
  ...
  useLayoutEffect(() => {
    ...
    // Con `glyphs` son varios <path> (un <clipPath> no admite agruparlos en <g>)
    const glyphNodes = Array.from(section.querySelectorAll('[data-gp-glyph]'));
    ...
    let W = 1, H = 1, travel = 1, startScale = 1, endScale = 1, anchorY = 0;
    ...
    stalled = !glyphs && available.length < families.length;

    const readInk = () => {
      if (!context) return false;
      if (glyphs) {
        const x0 = Math.min(...glyphs.map((g) => g.box.x)), y0 = Math.min(...glyphs.map((g) => g.box.y));
        const x1 = Math.max(...glyphs.map((g) => g.box.x + g.box.width)), y1 = Math.max(...glyphs.map((g) => g.box.y + g.box.height));
        bounds = { x: x0, y: y0, width: x1 - x0, height: y1 - y0 };
        center = { x: x0 + bounds.width / 2, y: y0 + bounds.height / 2 };
        const requested = focusChar ? text.indexOf(focusChar.normalize('NFC')) : -1;
        candidates = []; letters = [];
        let offset = 0;
        glyphs.forEach((g, i) => {
          const char = characters[i]?.char ?? g.char;
          letters.push({ index: offset, ...g.box });
          const found = interiorPath(context, g.d, g.box);
          if (found) candidates.push({ ...found, index: offset });
          offset += char.length;
        });
    ...
    const paint = (progress) => {
      ...
      const transform = `translate(${W / 2} ${anchorY + H * 0.04 * eased}) scale(${scale}) rotate(${roll}) translate(${-cx} ${-cy})`;
      ...
      const dx = W / 2 / scale, dy = (anchorY + H * .04 * eased) / scale;
      clip.setAttribute('transform', `scale(${scale}) rotate(${roll})`);
      const glyphTransform = `translate(${Math.cos(radians) * dx + Math.sin(radians) * dy - cx} ${-Math.sin(radians) * dx + Math.cos(radians) * dy - cy})`;
      for (const node of glyphNodes) node.setAttribute('transform', glyphTransform);
      ...
      section.style.setProperty('--gp-reveal', String(isStatic ? 1 : overlay ? smooth(0.72, 0.84, p) : smooth(0.78, 0.9, p)));
    ...
    const layout = () => {
      ...
      const fixedHeight = Number.isFinite(wordMaxHeight);
      startScale = Math.min(W * (fixedHeight ? 0.9 : 0.84) / bounds.width, (fixedHeight ? wordMaxHeight : wordHeight) / bounds.height);
      // Nunca más abajo que el centrado original.
      anchorY = Number.isFinite(wordTop)
        ? (fixedHeight ? wordTop + bounds.height * startScale / 2 : Math.min(H * .46, wordTop + bounds.height * startScale / 2))
        : H * .46;
      ...
      for (const button of buttons) {
        ...
        Object.assign(button.style, {
          ...
          top: `${anchorY + (letter.y - center.y) * startScale - Math.max(0, 44 - letter.height * startScale) / 2}px`,
          width: `${Math.max(1, letter.width * startScale)}px`,
          height: `${Math.max(44, letter.height * startScale)}px`,
        });
      }
      section.style.setProperty('--gp-word-top', `${anchorY - bounds.height * startScale / 2}px`);
      section.style.setProperty('--gp-word-bottom', `${anchorY + bounds.height * startScale / 2}px`);
      section.dataset.gpReady = 'true';
      const on = !motion.matches && browserFrameSeen && !stalled && !!target;
      section.dataset.gpMotion = on ? 'on' : 'off';
      setMotionOn(on);
    };
  ...
  }, [text, focusChar, interactive, fontFamily, weight, length, clipId, hasFront, wordTop, wordMaxHeight, overlay, glyphs]);
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <style>{`
        ...
        ${q} [data-gp-overlay]{box-sizing:border-box;position:absolute;inset:0;z-index:2;display:grid;align-content:center;color:var(--gp-foreground);opacity:var(--gp-reveal,0);pointer-events:none;overflow-wrap:anywhere;}
        ${q}[data-gp-motion=on] [data-gp-content]:empty{pointer-events:none!important;}
        ${q}[data-gp-entered=true] [data-gp-overlay],${q} [data-gp-overlay]:focus-within{opacity:1;pointer-events:auto;}
        ${q}:has([data-gp-overlay]:focus-within) [data-gp-field]{clip-path:none!important;}
        ${q}:has([data-gp-overlay]:focus-within) [data-gp-caption],${q}:has([data-gp-overlay]:focus-within) [data-gp-front]{opacity:0;}
      ...
      <div data-gp-pin>
        ...
        <svg data-gp-art aria-hidden="true" focusable="false">
          <defs>
            <clipPath id={clipId} clipPathUnits="userSpaceOnUse">
              {glyphs
                ? glyphs.map((g, i) => <path key={i} data-gp-glyph="" d={g.d} clipRule="evenodd" />)
        ...
        <div data-gp-choices role="radiogroup" aria-label="Elegí la letra por la que entrar" inert="">
          {characters.map(({ char, index }, i) => <button type="button" role="radio" aria-checked="false" tabIndex={-1} data-gp-letter={index} key={index} aria-label={`${char}, letra ${i + 1} de ${characters.length}`} />)}
        </div>
        <label data-gp-touch-picker><span style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clipPath: 'inset(50%)' }}>Letra de entrada</span><select data-gp-select defaultValue="">
        ...
        <div data-gp-caption>
          <span data-gp-hint aria-hidden="true">{interactive ? 'Deslizá para entrar.' : annotations ? 'Un pasaje a través de la tipografía' : ''}</span>
          <a data-gp-enter href={`#${uid}-content`}>{enterLabel}<span aria-hidden="true">↘</span></a>
        </div>
        {overlay && motionOn && <div data-gp-overlay>{children}</div>}
      </div>
      <div data-gp-content id={`${uid}-content`} tabIndex={-1}>
        {overlay && motionOn ? null : children ?? <div><h2 data-gp-default-title>Una letra se vuelve un lugar.</h2><p data-gp-default-copy>La forma se abre hacia lo que viene.</p></div>}
      </div>
    </section>
  );
}
